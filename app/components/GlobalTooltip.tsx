...
const GlobalTooltip: React.FC = () => {
  const { isVisible, message } = useTooltipStore();
  const [isInFirstSection, setIsInFirstSection] = useState(true);
  const tooltipRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    const handleScroll = () => {
      // Check if user is in first section (hero section)
      // Assuming hero section height is roughly the viewport height
  ...
  return (
    ...
    >
      <p className="text-xl font-mono line-clamp-6 whitespace-normal break-words">
        {message}
      </p>
      <div
